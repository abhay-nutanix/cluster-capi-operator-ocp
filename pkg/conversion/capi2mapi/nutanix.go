...
func (m machineAndNutanixMachineAndNutanixCluster) toProviderSpec() (*mapiv1.NutanixMachineProviderConfig, []string, field.ErrorList) {
	...
	// ImageLookup is not supported in MAPI, add warning if present
	if m.nutanixMachine.Spec.ImageLookup != nil {
		warnings = append(warnings, "ImageLookup field is not supported in Machine API and will be ignored")
	}
	...
	if m.nutanixMachine.Spec.Project != nil {
		...
		// Set empty project when nil to maintain consistency
	...
	var categories []mapiv1.NutanixCategory

	if len(categoriesIdentifiers) > 0 {
...
func (m machineSetAndNutanixMachineTemplateAndNutanixCluster) ToMachineSet() (*mapiv1beta1.MachineSet, []string, error) {
...
func convertNutanixResourceIdentifierToMapi(identifier *nutanixv1.NutanixResourceIdentifier) (*mapiv1.NutanixResourceIdentifier, field.ErrorList) {
	...
	case nutanixv1.NutanixIdentifierName:
		...
		case *identifier.Name == "":
			errors = append(errors, field.Invalid(field.NewPath("name"), *identifier.Name, "Name cannot be empty for Name type identifier"))
		default:
	...
	case nutanixv1.NutanixIdentifierUUID:
		...
		case *identifier.UUID == "":
			errors = append(errors, field.Invalid(field.NewPath("uuid"), *identifier.UUID, "UUID cannot be empty for UUID type identifier"))
		default:
			obj.UUID = identifier.UUID
		}

	case "":
		errors = append(errors, field.Required(field.NewPath("type"), "identifier type must be specified"))
	default:
		errors = append(errors, field.Invalid(field.NewPath("type"), identifier.Type, "invalid identifier type, must be 'name' or 'uuid'"))
	}
...
func convertNutanixResourceIdentifierToStorageMapi(id *nutanixv1.NutanixResourceIdentifier) (*mapiv1.NutanixStorageResourceIdentifier, field.ErrorList) {
	...
	case nutanixv1.NutanixIdentifierName:
		// Storage identifiers don't support name type in MAPI
		errors = append(errors, field.Invalid(field.NewPath("type"), id.Type, "storage resource identifiers only support UUID type"))
	default:
		errors = append(errors, field.Invalid(field.NewPath("type"), id.Type, "invalid identifier type, only UUID is supported for storage resources"))
	}
...
func convertNutanixVMDiskToMapi(disk *nutanixv1.NutanixMachineVMDisk) (*mapiv1.NutanixVMDisk, field.ErrorList) {
	...
	// Convert DataSource
	ds, errs := convertDataSource(disk.DataSource)
	errors = append(errors, errs...)
	...
	// Convert DeviceProperties
	ddp, errs := convertDeviceProperties(disk.DeviceProperties)
	errors = append(errors, errs...)
	...
	// Convert StorageConfig
	storage, errs := convertStorageConfig(disk.StorageConfig)
	errors = append(errors, errs...)
...
func convertDataSource(ds *nutanixv1.NutanixResourceIdentifier) (*mapiv1.NutanixResourceIdentifier, field.ErrorList) {
	errors := field.ErrorList{}
	if ds == nil {
		return nil, errors
	}

	mapped, errs := convertNutanixResourceIdentifierToMapi(ds)
	if len(errs) > 0 {
...
func convertDeviceProperties(dp *nutanixv1.NutanixMachineVMDiskDeviceProperties) (*mapiv1.NutanixVMDiskDeviceProperties, field.ErrorList) {
	errors := field.ErrorList{}
	if dp == nil {
		return nil, errors
	}
...
func convertStorageConfig(sc *nutanixv1.NutanixMachineVMStorageConfig) (*mapiv1.NutanixVMStorageConfig, field.ErrorList) {
	errors := field.ErrorList{}
	if sc == nil {
		return nil, errors
	}
	...
	for _, g := range *gpus {
		...
		case nutanixv1.NutanixGPUIdentifierDeviceID:
			...
			if g.DeviceID == nil {
				...
				obj.DeviceID = ptr.To(int32(*g.DeviceID)) //nolint:gosec
			}
...
func convertNutanixBootTypeToMapi(bootType nutanixv1.NutanixBootType) (mapiv1.NutanixBootType, field.ErrorList) {
	...
	case "":
		// Empty boot type defaults to Legacy in MAPI
		mapiBootType = mapiv1.NutanixLegacyBoot
	default:
		errors = append(errors, field.Invalid(field.NewPath("bootType"), bootType, "invalid boot type, must be 'Legacy' or 'UEFI'"))
	}
